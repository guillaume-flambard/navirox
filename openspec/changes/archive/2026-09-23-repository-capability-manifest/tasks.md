...
- [x] 1.1 Create `packages/discovery` (package.json, tsconfig.json) and add it to
  the root `tsconfig.json` references. Verify `corepack pnpm build` builds it.

## 2. Define the manifest

- [x] 2.1 Define the versioned `RepositoryCapabilityManifest` schema, the fact
  ...
  deterministic.
- [x] 2.2 Define the injected reader interface (`files`, `readText`) and the
  discovery entry point. Verify the module never touches the filesystem itself.

## 3. Classify

- [x] 3.1 Implement the classification (`eligible`, `eligible-with-deltas`,
...
- [x] 3.2 Detect the package manager and lockfiles for pnpm, npm, Yarn and Bun,
  and the workspace declarations for Nx and Turborepo. Verify a fixture per
  family.

## 4. Fixtures

- [x] 4.1 Add the positive, boundary and refused fixtures: a conventional single
...
- [x] 5.2 Run `corepack pnpm exec openspec validate
