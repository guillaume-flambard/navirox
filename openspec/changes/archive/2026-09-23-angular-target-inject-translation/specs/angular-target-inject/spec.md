...
How a bounded Angular `inject(Type)` field is translated into the emitted
native single-file component so template bindings on that inject target
resolve, and when the inject is refused with no source.
