...
- [x] 2.1 `examples/vue-basic/babel.config.js` takes `withVueFastRefresh` from `@memolabs-apps/metro-preset` and adds it to `plugins`, next to the existing local plugin, with a comment saying what the plugin does and why the file it matches is a re-labeled `.vue` rather than a `.vue`. Verify with `corepack pnpm --filter vue-basic lint` and by reading the file.
