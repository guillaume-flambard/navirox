/**
 * Detox e2e config for the Navirox canary.
 *
 ...
 * developed on.
 *
 * The emulator's screen matters, and not for looks: the canary is a fixed
 * `flex: 1` column whose list takes the height the card leaves, so a small AVD
 * starves the list to zero and the list journey fails for a reason that lives
 * in the device profile rather than in the code. Pick a phone-sized AVD.
 *
...
module.exports = {
  ...
  apps: {
    'ios.debug': {
      type: 'ios.app',
      binaryPath: 'ios/build/Build/Products/Debug-iphonesimulator/VueBasic.app',
      build:
        'xcodebuild -workspace ios/VueBasic.xcworkspace -UseNewBuildSystem=NO -scheme VueBasic -configuration Debug -sdk iphonesimulator -derivedDataPath ios/build',
