...
| V1   | Reproducible web and native capture harness                                                        | In progress: reproducible web capture and comparison are verified on the records fixture; the native side has no device driver, so every iOS and Android capture is recorded as unavailable |
| V2   | One Vue scenario passes states, layout/style tolerances and motion frames on iOS and Android       | Not started: it needs the native captures V1 does not yet produce                                                                                                                           |
