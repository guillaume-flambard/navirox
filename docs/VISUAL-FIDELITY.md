...
| V2   | One Vue scenario passes states, layout/style tolerances and motion frames on iOS and Android       | Complete for the records fixture: both scenarios declare a cross-platform tolerance and the comparison returns a verdict. One dispatched run captures both platforms and both pass (iOS grid difference 0.00 percent, Android 2.54 percent inside the declared 5 percent, all four declared identifiers asserted, motion labels in order). The raw pixel ratio stays a measurement. See `docs/evidence/records-native-capture.md` |
